...
6. React/`localStorage` must never become the source of truth for agent task state, approval state, execution state, memory, or protocol events. UI preferences (panels, last workspace, display settings) may be cached locally. **Current backend source of truth:** workspace-local task files at `{workspace}/.codra/tasks/*.json` and events at `.codra/tasks/events/*.jsonl` (via `codra-core` `TaskStore`). `codra-memory` / SQLite is planned, not implemented for the task loop.
