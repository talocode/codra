...
import { addPendingEdit, getPendingEdits, applyPendingEdits, discardPendingEdits } from '../session/pending.js';

export async function appendCommand(args: string[]) {
  if (args.length < 2) {
...
export async function patchCommand(args: string[]) {
  if (args.length < 2) {
