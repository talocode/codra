...
import type { DetectedStack, PackageManager, ProjectType } from './types.js';
...
function detectProjectType(cwd: string, pkg: Record<string, unknown> | null): ProjectType {
  const deps = { ...(pkg?.dependencies as Record<string, unknown> || {}), ...(pkg?.devDependencies as Record<string, unknown> || {}) };
  const depNames = Object.keys(deps);

  if (fileExists(path.join(cwd, 'Cargo.toml')) && !fileExists(path.join(cwd, 'package.json'))) return 'rust';
  if (fileExists(path.join(cwd, 'src-tauri'))) return 'tauri';
  if (depNames.includes('remotion') || depNames.includes('@remotion/cli')) return 'remotion-video';
  if (depNames.includes('next')) return 'nextjs';
  if (depNames.includes('vite') && depNames.includes('react')) return 'react-vite';
  if (depNames.includes('react') && depNames.includes('react-dom')) return 'node-typescript';

  const scripts = pkg?.scripts as Record<string, string> || {};
  if (scripts.bin || pkg?.bin) return 'cli';

  if (fileExists(path.join(cwd, 'pnpm-workspace.yaml')) || fileExists(path.join(cwd, 'lerna.json'))) return 'monorepo';

  if (pkg?.bin || scripts.start?.includes('node') || scripts.start?.includes('tsx')) return 'cli';
  if (fileExists(path.join(cwd, 'Cargo.toml'))) return 'rust';
  if (fileExists(path.join(cwd, 'tsconfig.json'))) return 'node-typescript';

  return 'unknown';
}

export function analyzeProject(cwd: string): DetectedStack {
  ...
  return {
    ...
    detectedFrameworks: depNames.filter(d =>
      ['next', 'react', 'remotion', 'express', 'fastify', 'hono', 'nuxt', 'svelte', 'vue', 'tailwindcss'].includes(d)
    )
  };
}
