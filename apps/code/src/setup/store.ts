...
function getSetupDir(cwd: string): string {
  return path.join(cwd, SETUP_DIR);
}

export function saveSetupReport(report: SetupReport): void {
...
export function loadSetupReport(cwd: string): SetupReport | null {
...
function formatReportMarkdown(report: SetupReport): string {
  const lines: string[] = [];
  ...
  if (report.recommendation.risks.length > 0) {
    lines.push('');
    lines.push('## Risks');
    for (const r of report.recommendation.risks) lines.push(`- ${r}`);
  }
