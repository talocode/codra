...
export function formatVisualPlanMarkdown(plan: VisualPlan): string {
  const lines: string[] = [];
  ...
  if (plan.planId) lines.push(`**Linked Plan:** ${plan.planId}`);
  lines.push('');
  ...
  if (plan.approval.notes) lines.push(`**Notes:** ${plan.approval.notes}`);
  lines.push('');
  
  return lines.join('\n');
}

export function formatVisualPlanConsole(plan: VisualPlan): string {
  const lines: string[] = [];
  
  lines.push(chalk.cyan(`\n  Visual Plan: ${plan.title}`));
  lines.push(chalk.gray(`  ID: ${plan.id}`));
  lines.push(chalk.gray(`  Status: ${plan.status}`));
  if (plan.planId) lines.push(chalk.gray(`  Plan: ${plan.planId}`));
  lines.push('');
